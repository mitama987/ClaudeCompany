...
### Step 3.5: アイキャッチ画像（サムネイル）アップロード
記事トップのサムネイル画像は、エディタ上部の「画像を追加」ボタンからアップロードする。

1. `find` で「画像を追加」ボタンを探してクリック
2. 画像アップロードパネルが表示される → 「画像をアップロード」ボタンをクリック
3. `input[type="file"]#note-editor-eyecatch-input` が出現する
4. **自動アップロードの制限**: ブラウザのセキュリティ制限により、ローカルファイルのfile inputへの自動セットは困難。以下のいずれかで対応:
   - ユーザーに手動でファイル選択ダイアログからアップロードを依頼
   - chrome-devtools MCP の `upload_file` が使える場合はそちらを使用（同じブラウザインスタンスである必要あり）
