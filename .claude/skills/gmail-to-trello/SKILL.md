...
## 関連ワークフロー: blue_dark カードのモチベーション補強（再分解＋偉人の名言）

「やる気が起きない」「タスクが粗くて手がつかない」と言われたとき、blue_dark ラベルの今日タスクを以下の手順で補強する。
本スキルが Gmail から作成するカードは blue_dark ラベルが付くため、後日この補強ワークフローの対象になりやすい。

### トリガー
- 「blue_dark のタスクのやる気が出ない」「やる気起きない」
- 「今日のタスクを再分解して」
- 「モチベ上げて」「名言入れて」「偉人の名言で」

### 対象カードの特定
- 既定: 「今日（前半）」(`5ffe5b3fa17b04881f2c4fbf`) 内の **blue_dark ラベル** (`5ffe5b40a17b04881f2c51a6`) 付きカード
- 複数該当した場合は `AskUserQuestion` で対象を選ばせる（「全部まとめて」も選択肢に入れる）
- ユーザー指定のカード／リストがあればそちらを優先

### 実行手順（カード単位で直列実行）

#### Step A: 再分解チェックリスト追加

1. `mcp__trello__create_checklist` で新規チェックリスト名 `再分解：<元のタスク名>` を作成
   - **既存チェックリストには触らない**（idChecklists の既存IDをそのまま残す）
2. `mcp__trello__add_checklist_item` で「次の一歩」が分かる小タスクを **5〜8項目** 追加
   - 各項目は1行で完結する具体的な行動
   - **時間 or 数量を入れる**（例: "〜を3つ書く" "〜を1分で決める" "5分で殴り書き"）
   - **最初の項目は最も心理的ハードルが低いもの**（例: "インストール" "1行書く" "サイズ確認"）
   - `cardId` を必ず指定して曖昧さを回避
   - 順次直列実行（並列しない）

#### Step B: 偉人の名言コメント追加

`mcp__trello__add_comment` で以下のフォーマットで投稿：
...
<2〜3行のメッセージ：障壁の言語化＋最初の1ステップへの背中押し>
```
...
| テーマ | 推奨名言の方向性 | 例（著者） |
|--------|------------------|-----------|
| 環境整備・準備系 | 準備の価値、整理の重要性 | イチロー、ジョン・C・マクスウェル |
| 発信・露出が怖い | 行動の優位性、後悔の重さ | マーク・トウェイン、ウォルト・ディズニー |
| 完璧主義で止まる | まず終わらせる、不完全許容 | マーク・ザッカーバーグ、アンリ・ベルクソン |
| 大きすぎて手がつかない | 小さく始める、千里の道 | 老子、孔子 |
| 失敗が怖い | 失敗の価値、リスク許容 | エジソン、本田宗一郎 |
| 継続が辛い | 積み重ねの力 | 松下幸之助、二宮尊徳 |

- 同じ名言を連続で使い回さない（カードごとに別の組合せにする）
...
```
| カード | 追加チェックリスト | 名言コメント |
|---|---|---|
| [カード名](shortUrl) | 再分解：xxx — N項目 | 著者A / 著者B |
```

最後に「最初の1ステップだけまずポチッと」のような背中押しを1行添える。

### 触らないもの
- 既存チェックリスト
- カードの name / desc / due / labels / pos
- リスト位置・カード順序
...
- ver1.7 | 2026-04-19 | 関連ワークフローとして「blue_darkカードの再分解＋偉人の名言」補強手順を追加。やる気が起きない時の対処をテンプレ化
